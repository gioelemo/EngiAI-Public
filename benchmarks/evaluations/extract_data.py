...
def _extract_metrics_from_scorers(  # noqa: PLR0912, PLR0915
    output_quality: dict,
    task_completion: dict,
    tool_use: dict,
    problem_type: str | None = None,
    ...
    if isinstance(output_quality, dict):
        ...
        for key in list(output_quality.keys()):
            if key.endswith(constraint_suffixes):
                try:
                    value = output_quality[key]
        ...
        for key in list(output_quality.keys()):
            if (
                key.startswith(objective_prefixes) or key.endswith(objective_suffixes)
    ...
    if isinstance(tool_use, dict):
        ...
        if isinstance(tool_breakdown, dict):
            for tool_name in list(tool_breakdown.keys()):
                try:
                    result[f"tool_{tool_name}"] = tool_breakdown[tool_name]
                except (KeyError, TypeError):
                    continue

        # Also extract any individual tool usage fields (tool_*)
        for key in list(tool_use.keys()):
...
def _process_score_call_for_complete_data(  # noqa: PLR0911, PLR0912, PLR0913, PLR0915
    ...
    try:
        ...
        logger.warning("Failed to process score call: %s", exc)
        traceback.print_exc()
        return None
    else:
